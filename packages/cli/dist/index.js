...
const DEFAULT_REGISTRY_URL = "https://raw.githubusercontent.com/PattemChaitanya/material-tail/main/packages/registry/dist/registry.json";
// ============================================================
...
async function fetchRegistry(registryUrl) {
    if (registryUrl.startsWith("http://") ||
        ...
        if (!response.ok) {
            throw new Error(`Failed to fetch registry from ${registryUrl}: ${response.statusText}`);
        }
...
program
    .name("material-tail")
    .description("CLI to copy custom CSS Modules components into your React project")
    .version("1.0.0");
// ============================================================
// COMMAND: init
// ============================================================
program
    ...
    if (!options.yes) {
        ...
        if (response.libPath !== undefined)
            libPath = response.libPath;
        if (response.stylesPath !== undefined)
            stylesPath = response.stylesPath;
        if (response.registryUrl !== undefined)
            registryUrl = response.registryUrl;
    }
    const config = { componentsPath, libPath, stylesPath, registryUrl };
    // 1. Write config file
    ...
    if (existingCssContent.includes("material-tail-theme-start")) {
        ...
        fs.writeFileSync(resolvedCssPath, existingCssContent + separator + DEFAULT_THEME_CSS, "utf-8");
...
program
    ...
    function resolve(name) {
        ...
        if (!entry) {
            ...
            console.log(`Available: ${registry.map((e) => e.name).join(", ")}\n`);
            process.exit(1);
        }
    ...
    for (const componentName of toInstall) {
        ...
        for (const file of entry.files) {
            ...
            console.log(`  ${exists ? "↺" : "+"} ${path.relative(process.cwd(), destFilePath)}`);
        }
    }
    console.log("\n✓ Done!\n");
});
...
program
    ...
    for (const entry of registry) {
        ...
        const deps = entry.registryDependencies?.length > 0
