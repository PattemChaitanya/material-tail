...
const DialogTitle = styled.h2<DialogTitleStyledProps>`
  margin: 0;
  font-size: 1.25rem;
...
const DialogContent = styled.div<DialogContentStyledProps>`
  padding: 24px;
  overflow-y: auto;
  flex: 1;
...
interface DialogActionsStyledProps {
  theme: Theme;
}

const DialogActions = styled.div<DialogActionsStyledProps>`
  display: flex;
  align-items: center;
...
export const DialogTitleComponent: React.FC<DialogTitleProps> = ({
  children,
}) => {
  const theme = useTheme();
  return <DialogTitle theme={theme}>{children}</DialogTitle>;
};

export const DialogContentComponent: React.FC<DialogContentProps> = ({
  children,
}) => {
  const theme = useTheme();
  return <DialogContent theme={theme}>{children}</DialogContent>;
};

export const DialogActionsComponent: React.FC<DialogActionsProps> = ({
  children,
}) => {
  const theme = useTheme();
  return <DialogActions theme={theme}>{children}</DialogActions>;
};
